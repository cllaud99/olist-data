...
StateType = Literal['AC', 'AL', 'AP', 'AM', 'BA', 'CE', 'DF', 'ES', 'GO', 'MA', 'MT', 'MS', 'MG', 'PA', 'PB', 'PR', 'PE', 'PI', 'RJ', 'RN', 'RS', 'RO', 'RR', 'SC', 'SP', 'SE', 'TO']
...
class OlistCustomer(ConfigBase):
    ...
    customer_unique_id: str = Field(..., description="Identificador único por cliente, considerando que um cliente pode ter mais de um ID")
    customer_zip_code_prefix: str = Field(..., description="Prefixo do código postal do cliente")
...
class OlistGeolocation(ConfigBase):
    geolocation_zip_code_prefix: str = Field(..., description="Prefixo do código postal da geolocalização")
...
class OlistOrderPayment(ConfigBase):
    ...
    payment_type: Literal['credit_card', 'boleto', 'voucher', 'debit_card', 'not_defined'] = Field(..., description="Tipo de pagamento")
    payment_installments: int = Field(..., description="Número de parcelas do pagamento")
...
class OlistOrderReview(ConfigBase):
    ...
    review_creation_date: datetime = Field(..., description="Data de criação da avaliação")
    review_answer_timestamp: Optional[datetime] = Field(None, description="Data e hora da resposta da avaliação")

class OlistOrder(ConfigBase):
    order_id: str = Field(..., description="Identificador único do pedido")
    customer_id: str = Field(..., description="Identificador único do cliente")
    order_status: str = Field(..., description="Status do pedido")
    order_purchase_timestamp: datetime = Field(..., description="Data e hora da compra do pedido")
    order_approved_at: Optional[datetime] = Field(None, description="Data e hora da aprovação do pedido")
    order_delivered_carrier_date: Optional[datetime] = Field(None, description="Data em que o pedido foi entregue ao transportador")
    order_delivered_customer_date: Optional[datetime] = Field(None, description="Data em que o pedido foi entregue ao cliente")
    order_estimated_delivery_date: Optional[datetime] = Field(None, description="Data estimada de entrega do pedido")
...
class OlistProduct(ConfigBase):
    ...
    product_name_length: int = Field(..., alias="product_name_length", description="Comprimento do nome do produto em caracteres")
    product_description_length: int = Field(..., alias="product_description_length", description="Comprimento da descrição do produto em caracteres")
    ...
    product_length_cm: int = Field(..., description="Comprimento do produto em centímetros")
